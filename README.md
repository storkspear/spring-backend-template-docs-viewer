# spring-backend-template-docs-viewer

[`spring-backend-template`](https://github.com/storkspear/spring-backend-template) 의 `docs/` 를 **웹에서 보기 위한 static site** 입니다.
`docs/` 원본은 이 레포 안에 복사본으로 들어있고, SPA (index.html + assets/*) 가 마크다운을 런타임에 렌더링합니다.

## 🌐 라이브 URL

**<https://storkspear.github.io/spring-backend-template-docs-viewer/>**
...
└── docs/
    ...
    └── **/*.md           # spring-backend-template 에서 복사된 문서
```
...
1. **원본 수정**: `spring-backend-template/docs/` 에서 먼저 수정 후 push
...
- 원본 레포: [`spring-backend-template`](https://github.com/storkspear/spring-backend-template)
- 자매 뷰어: [`flutter-mobile-template-docs-viewer`](https://storkspear.github.io/flutter-mobile-template-docs-viewer/)
