...
앱 추가 시 4 provider 소셜 로그인 (Google · Apple · Kakao · Naver) 을 위한 credential 설정 방법이에요.

> 이 문서는 *콘솔 화면 단계까지 자세하게* 4 provider 의 OAuth 자격을 발급·등록하는 절차를 다뤄요. 운영 환경에서 발급해야 하는 다른 키들 (Cloudflare / Resend / PortOne / Apple IAP / Google IAP / MinIO 등) 의 발급 목적과 절차를 한 곳에서 보고 싶다면 [`운영 키 발급 통합 가이드`](../production/setup/key-issuance.md) 를 먼저 확인하세요. 이 가이드는 그 통합 가이드의 §4.3 (소셜 로그인) 의 *상세 보충* 이에요.
...
> **활성화하지 않은 provider 는 이 가이드의 해당 섹션 전부 생략 가능해요**. 예를 들어 글로벌 시장 앱이면 Kakao/Naver 섹션은 무시하고 Google + Apple 만 따라가면 돼요.

> **OAuth 키 발급 전 e2e 시연** 이 필요하면 [dev-mock 모드 섹션](#oauth-키-발급-전-e2e-시연-dev-mock-모드) 을 먼저 참조하세요. WireMock 컨테이너로 4 provider 를 통째 가짜로 띄워 키 없이도 백엔드 → JWT 발급 흐름을 검증할 수 있어요.
...
4. 이 프로젝트는 모든 앱에서 공유해요. 앱마다 새로 만들 필요가 없어요.
...
5. **앱 게시** (선택) — 테스트 모드에서는 등록된 테스트 사용자만 로그인할 수 있어요. 출시 전에 "프로덕션으로 푸시"를 눌러야 모든 사용자가 로그인할 수 있어요.

### 3단계: 앱별 OAuth 클라이언트 ID 발급

앱 하나당 **iOS용 1개 + Android용 1개 = 2개** 를 만들어야 해요. Flutter 앱이 iOS/Android 각각에서 Google 로그인할 때 플랫폼별 Client ID 가 다르기 때문이에요.

콘솔 위치는 https://console.cloud.google.com → **APIs & Services** → **사용자 인증 정보** 예요.
...
6. 화면에 표시되는 **클라이언트 ID** 를 복사해요
   ...
   - 이 값이 `.env.prod` 의 `GOOGLE_CLIENT_IDS_0` 에 들어가요
...
5. **SHA-1 인증서 지문** 입력:
   ...
   - 또는 디버그용은 개발 중에만 쓰고, 릴리스용만 `.env.prod` 에 등록해도 돼요
...
서버는 Google 토큰의 `aud` 값이 이 리스트에 포함되는지 검증해요. iOS 기기에서 로그인하면 `aud` 가 iOS Client ID, Android 에서 로그인하면 Android Client ID 가 돼요.
...
5. 입력:
   ...
   - Bundle ID: **Explicit** 선택 → `com.twosun.sumtally` 입력
     - 이 값은 Xcode 와 Flutter 프로젝트에서 사용하는 Bundle Identifier 와 반드시 동일해야 해요
6. Capabilities 목록에서 **Sign In with Apple** 체크
7. **Continue** → **Register**

> 이미 Xcode 에서 앱을 만들면서 App ID 가 자동 등록된 경우, Identifiers 목록에서 해당 앱을 클릭하고 Capabilities 에서 **Sign In with Apple** 을 체크하면 돼요.
...
서버는 Apple identity token 의 `aud` 값이 이 Bundle ID 와 일치하는지 검증해요.

> Apple Sign In 은 iOS 기기에서만 사용돼요 (Android 에서는 Google Sign In 만 제공).

---

## Kakao Sign In

> 한국 시장 앱이면 거의 항상 활성화해요. 글로벌 전용이면 이 섹션을 생략하세요.

### ⚠️ Kakao 만 키 2개 — 헷갈림 주의

카카오 디벨로퍼스 콘솔의 **같은 앱 등록 페이지**에서 두 개의 식별자가 발급돼요.
...
콘솔 대시보드에 둘 다 나란히 표시돼요. **둘 다 복사해서 각자 자리에 등록**해야 동작해요. 한 쪽만 등록하면 검증에 실패해요.
...
4. **동의 항목** → 닉네임 + 이메일 활성화 (이메일은 "선택" 또는 "필수")
   - "필수" 권장 — 백엔드가 이메일 누락 시 `email_required` 401 응답을 내요
...
서버는 Kakao access token 으로 `/v1/user/access_token_info` 호출 → 응답의 `app_id` 가 이 값과 일치하는지 검증해요.

> 프론트 (Flutter) 에는 Native App Key (문자열) 가 따로 들어가요 — `template-flutter` 의 `auth-kit.md` 참조.

---

## Naver Sign In

> 한국 시장 + 30~50대 포털 사용자 비중이 높을 때만 추가하세요. 20~30대 모바일 위주면 보통 Kakao 만으로 충분해요.
...
서버는 Naver access token 으로 `/v1/nid/me` 호출 → 응답 `resultcode=00` + 이메일 검증을 해요. Naver 가 자체적으로 토큰 발급 client 검증을 하므로 (다른 client 토큰은 401) Client Secret 은 백엔드에 등록하지 않아요.

---

## OAuth 키 발급 전 e2e 시연 (dev-mock 모드)

파생 레포 만든 첫날, 위 4 provider 의 콘솔 작업이 **하나도 안 된 상태**에서도 백엔드 + 프론트 종단 흐름을 시연할 수 있어요. WireMock standalone 컨테이너가 Google/Kakao/Naver 의 4 endpoint 를 가짜 응답으로 stub 하고, Apple 만 별도 `MockAppleSignInService` 가 RS256 검증을 우회해요.
...
`app.oauth.dev-mock=true` 가 `MockAppleSignInService` 를 활성화해요 — 어떤 identity_token 이 와도 고정 fake user (`dev-apple-mock-user` / `dev-apple@example.com`) 로 통과시켜요.
...
프론트의 `DevMock*Gate` 가 즉시 dummy 토큰을 반환 → 백엔드 → WireMock 통과 → JWT 발급 → /home 자동 리다이렉트가 일어나요.
...
→ **운영 빌드는 영향이 0 이에요**. prod profile 은 wiremock URL 환경변수 미주입 시 실 provider URL 로 fallback 하는 안전망을 가져요.

### 안전 확인용 부팅 로그

dev-mock 모드 활성 시 다음 WARN 로그가 출력돼요.
```
WARN  MockAppleSignInService activated — Apple RS256 verification is BYPASSED.
      DO NOT enable this in production.
```

이 로그가 운영 환경에서 보이면 즉시 셧다운 + 환경변수 점검을 해요.

---

## 앱 추가 체크리스트

새 앱 `my-new-app` 을 추가할 때의 체크리스트예요.
...
> 4-stage secret chain 동기화 — `.env.prod` 에 추가만 한다고 운영에 자동 반영되지 않아요. `config/deploy.yml` / `.kamal/secrets.example` / `.github/workflows/deploy.yml` 까지 4 곳 모두에 같은 키를 명시해야 해요. 자세한 체크리스트는 [`secret chain 4-stage 통합 가이드`](../production/setup/secret-chain-4stage.md) 를 참조하세요.
...
> **환경변수 네이밍 규칙** — 앱 slug 는 대문자로, 하이픈은 언더스코어로 변환해요.
...
아니요. 프로젝트당 1회 설정이면 충분해요.

### Q: 개발 중에는 어떻게 테스트하나요?

`application-dev.yml` 에 dev 용 credential 이 설정되어 있어요. 실제 Google/Apple 로그인을 테스트하려면 해당 값을 실제 발급받은 Client ID 로 교체하세요.

### Q: 코드를 수정해야 하는 경우가 있나요?

없어요. `AppCredentialProperties` 가 환경변수를 `Map<String, AppCredential>` 로 자동 바인딩하고, 서비스가 요청의 `appSlug` 로 조회해요. 환경변수 추가 + 재배포만 하면 돼요.

### Q: Kakao 의 키 두 개가 헷갈려요. 어디에 어느 걸 넣나요?

콘솔에서 발급되는 두 키의 위치가 달라요.
...
둘 다 같은 카카오 콘솔 대시보드에서 나란히 보여요. 한 쪽이라도 빠지거나 바뀌면 동작이 안 돼요.

### Q: dev-mock 모드는 운영 빌드에 영향이 있나요?

없어요. `app.oauth.dev-mock=true` 환경변수가 명시적으로 주입돼야만 `MockAppleSignInService` 가 활성화돼요. prod profile 에서는 wiremock URL 환경변수도 미주입이 정상이라 `application.yml` 의 default (실 provider URL) 로 fallback 돼요.

### Q: dev-mock 모드 활성 여부는 어떻게 확인하나요?

부팅 로그에서 다음 WARN 한 줄이 나오면 dev-mock 모드예요.
```
WARN  MockAppleSignInService activated — Apple RS256 verification is BYPASSED.
```
운영 환경에서 이 로그가 보이면 즉시 셧다운 + `APP_OAUTH_DEV_MOCK` 환경변수 점검을 해요.
