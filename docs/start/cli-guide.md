...
| verb | dev | prod | 비고 |
|---|---|---|---|
| **migrate** `<slug> <V*>` | dev DB 에 V스크립트 적용 (`.env.dev` 의 JDBC_DB_URL 사용) | prod DB 에 V스크립트 직접 적용 (ADR-033 Hybrid). `--dry-run` / `--force` 지원 | [`flyway-runbook`](../production/deploy/flyway-runbook.md) — `migrate-prod.sh --target={dev,prod}` |
...
| **force-clear** `[slug]` | ⚠ `cleanup` + dev Supabase 스키마 DROP + dev MinIO bucket 제거. **3단계 confirm** + prod host 충돌 safety check (`.env.dev` 의 JDBC_DB_URL host 가 `.env.prod` 와 같으면 즉시 abort) | ⚠ `clear` (인프라) + 데이터 + 관측성까지 *모두* 영구 삭제. `[slug]` 생략 시 모든 앱 + core. **5단계 confirm** — 한 단계라도 'y' 외 입력 시 즉시 abort | dev 백업 모드 없음 (외부 Supabase 는 콘솔에서 직접 백업) / prod 자동 백업 미구현 (manual 안내만) |
...
`.env.dev` 의 REQUIRED 키 (`BASE_DOMAIN`, `SUBDOMAIN_DEV`, `JDBC_DB_URL` — 별도 Supabase, `APP_STORAGE_MINIO_BUCKETS_0` — dev bucket) 를 채운 뒤에 진행해요. Cloudflare / Tailscale / SSH / MinIO ENDPOINT·KEY / Resend 등은 prod 와 공용 secret.
