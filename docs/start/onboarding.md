...
`tools/new-app/new-app.sh <slug> --provision-db` 는 어떤 provider 여도 동일한 표준 `psql` 을 호출해요. 결정해야 할 것은 **`DATABASE_URL`** (admin role, schema/role 생성 용) 한 줄과 **`DB_URL` / `DB_USER` / `DB_PASSWORD`** (앱 런타임 credential) 예요.
...
> **`DATABASE_URL` 이 뭔가요?** schema 와 role 을 **생성할 관리자 권한** connection string 이에요.
> 앱 전용 `GYMLOG_DB_URL` (앱 role `gymlog_app` 로 접속) 과는 다른 개념 — 수퍼유저/관리자 credential 이 필요해요.
>
> - **로컬 dev** (docker-compose postgres): **별도 설정 불필요** — `.env.example` 에 `DATABASE_URL=postgresql://postgres:dev@localhost:5433/postgres` 기본값이 있고, `new-app.sh` 가 `.env` 에서 자동 로드해요. 로컬 docker 환경은 결정적이라 사용자 조작이 없어요.
...
> ```
>
> `--provision-db` 없이 수동 실행도 가능해요: `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f infra/scripts/init-app-schema.sql`
