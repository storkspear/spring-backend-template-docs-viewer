...
> 시간 순 walkthrough: [`도그푸딩 walkthrough`](./dogfood-walkthrough.md)
...
### Q2. cleanup 한 후 setup 을 다시 반복해도 되나요?

**OK 예요**. 두 스크립트 모두 멱등성을 보장해요.

- `setup.sh`: `gh secret set` 은 overwrite 동작이고, `authorized_keys` 는 grep 후 append 라 중복이 추가되지 않아요
- `cleanup.sh`: 없는 자원을 삭제하려고 시도하면 `[WARN] ... 없음 (skip)` 만 출력하고 graceful 하게 진행돼요

검증을 여러 번 돌려서 자동화 안정성을 확인하는 워크플로우를 권장해요.

---

### Q3. PAT 안 쓰고 `GITHUB_TOKEN` 으로만 가능한가요?

**현재 안 돼요** (2026-04 기준). 이유는 [`pitfalls #5 ~ #7`](./dogfood-pitfalls.md) 에 정리돼 있어요.

- 첫 GHCR 패키지 생성 시 repo↔package 자동 연결이 안 돼서 GITHUB_TOKEN 으로 push 시 403 이 나요
- `workflow permissions = write` + `provenance/sbom: false` 다 적용해도 동일하게 실패해요

GitHub 측에서 이 동작이 개선되면 PAT 폐기가 가능해요 ([`I-10 결정 카드의 재검토 트리거`](../production/deploy/decisions-infra.md)).
...
- `ci.yml` (push, PR) 은 항상 동작해요 — 분 사용 (CI 5분/회)
- `deploy.yml` 은 `workflow_run` 으로 트리거되지만 gate 가 즉시 skip 해서 **2~3초** 만에 종료돼요. billed ~0.05분 정도예요
- 즉 CI 비용은 그대로지만 deploy 는 사실상 무시할 만해요

template 상태에서 DEPLOY_ENABLED 가 기본 미설정인 이유예요.

---

### Q5. Mac mini 가 아니라 다른 호스트 (e.g. 클라우드 VPS) 면요?

**가능해요**. 변경 포인트는 다음과 같아요.

- `.env.dogfood` 의 `DEPLOY_HOST`, `DEPLOY_SSH_USER` 만 그 호스트에 맞춰 변경하세요
- 그 호스트에 docker / kamal-proxy 가 기동 가능해야 해요 (ARM64 이미지를 풀 수 있어야 해요)
- 만약 x86 호스트면 `.github/workflows/deploy.yml` 의 `platforms: linux/arm64` 를 `linux/amd64` 로 변경하세요 (template 결정 [`I-04`](../production/deploy/decisions-infra.md) 와 충돌하므로 별도 ADR 이 필요해요)
- Tailscale 로 도달 가능한 호스트면 OAuth 셋업 그대로 사용하세요
- Tailscale 안 쓰면 `tailscale-action` step 제거 + 호스트 public IP 또는 다른 VPN 셋업이 필요해요

---

### Q6. cleanup 후 Mac mini 의 `kamal-proxy` 컨테이너가 사라졌어요. 다음 배포가 또 setup 부터 인가요?

**네**. 다음 `setup.sh` + 첫 배포 시 kamal 이 자동으로 setup (kamal-proxy 컨테이너 + docker network) 을 다시 해요 (~30초 정도).

매번 setup 시간이 부담이면 `bash tools/dogfooding/cleanup.sh --keep-proxy` 로 컨테이너를 유지할 수 있어요. 메모리 ~15MB / 디스크 ~150MB 정도가 추가로 사용돼요.

---

### Q7. `.env.dogfood` 가 실수로 commit 되면요?

**즉시 행동하세요**.

1. (가능하면) 새 commit 으로 파일 삭제 + push
2. **모든 키 즉시 폐기 + 재발급** ([`key-rotation.md`](../production/setup/key-rotation.md))
   - GitHub history 에 남으니 못 본 척 하면 안 돼요 — 이미 노출됐다고 가정하세요
3. (선택) `git filter-repo` 또는 BFG 로 history 재작성 + force push

**예방 방법**:
- `.gitignore` 에 `.env.*` 가 잡고 있어요
- `setup.sh` 시작 시 `git check-ignore` 로 검증해요 (실수 가능성을 줄여요)
- pre-commit hook 으로 `.env.dogfood` 패턴을 차단할 수 있어요 (option)
...
- cloudflared tunnel 이 연결되어 있으면 외부에서 `https://server.<도메인>` 로 접근이 가능해요

**하지만 도그푸딩 의도와는 맞지 않아요 — 권장하지 않아요**.

- DB/secrets 가 dummy 또는 테스트 값일 수 있어요
- 도그푸딩은 "한 사이클 검증" 용이에요. 본격 운영은 별도 변수 / 별도 도메인 / 별도 인프라를 권장해요

---

### Q9. setup.sh 가 중간에 fail 하면 어디서부터 재실행해야 하나요?

**처음부터 그냥 재실행해도 OK 예요**. 멱등성 설계라서 다음과 같이 동작해요.
...
→ fail 한 step 부터 다시 시작하지 않고 그냥 `bash tools/dogfooding/setup.sh` 한 번 더 돌리면 돼요.

---

### Q10. 외부 도메인 없이 localhost 로 검증만 가능한가요?
...
도메인 (PUBLIC_HOSTNAME) 은 placeholder 라도 상관없어요 — kamal-proxy 의 host-based routing 에 사용돼서 curl 의 Host 헤더와 일치하면 돼요. 외부 인터넷 접근만 cloudflared 가 필요해요.

---

### Q11. 11회 시도했다는데 다시 셋업하면 또 11번 걸리나요?

**1번에 끝나요**. 11회 시도 중 8 함정은 워크플로우/스크립트 코드에 박혀서 영구 회피되고, 3개는 외부 발급 (PAT / Tailscale OAuth / DB URL 형식) 이라서 가이드 §3 에 정확한 절차 + 함정 강조가 들어 있어요. 12 번째 JDK 26 함정 ([`pitfalls #12`](./dogfood-pitfalls.md)) 만 사람이 JDK 21 환경을 보장해야 해요.

→ 가이드 따라 1번에 setup → 자동 trigger → 배포 success 가 정상 흐름이에요.

(만약 새로운 함정을 만나면 [`pitfalls.md`](./dogfood-pitfalls.md) §"새 함정 발견 시" 절차로 추가 PR 을 올리세요.)
...
<repo-name> local start # docker compose + postgres ready 만
...
| 7 | OPTIONAL: alertmanager | Alertmanager 컨테이너 Up 확인 | Discord 도착은 기술적으로 검증 불가 — 알람을 수동 trigger 후 채널에서 확인 |
...
| `.env.prod` 존재 + REQUIRED 5 비어있음 | **1회차 직후 (사용자가 채우는 중)** | Step 6 에서 안내만 출력 후 종료 (Step 7~11 도달 안 해요) |
...
| **`RESEND_TEST_ADMIN_USER_EMAIL`** | **`verify-server.sh` 검증만** | ✗ (의도된 제외) | ✗ |

`verify-server.sh` 를 운영 환경에서 SSH 로 실행할 때 `.env.prod` 를 직접 source 하므로 GitHub Secrets 에 push 할 필요가 없어요. 비어있으면 Step 5 가 자동 SKIP — 운영 동작에 영향이 없어요.

채우려면 `.env.prod` 에 본인 이메일을 직접 입력하세요 (Secrets push 가 안 돼요).
...
**즉 GitHub Secrets 에 push 됐다고 해서 운영에서 동작하는 게 아니에요.** `config/deploy.yml` + `.kamal/secrets` 의 env.secret 목록에 같은 키를 명시해야 Kamal 이 환경변수로 컨테이너에 전달해요. 4-stage 동기화의 자세한 체크리스트는 [`secret chain 4-stage 통합 가이드`](../production/setup/secret-chain-4stage.md) 에서 확인하세요.

이 두 파일은 **파생 레포가 직접 append** 해야 해요 (template 의 두 파일에 코멘트로 "Phase 2 자동 append 예정" 명시 — 현재는 미구현). 새 앱 모듈을 `tools/new-app/new-app.sh <slug>` 로 추가한 뒤 다음 두 파일을 수정해요.
...
또 `.github/workflows/deploy.yml` 의 env 블록에도 같은 키를 `${{ secrets.APP_CREDENTIALS_<SLUG>_* }}` 형태로 export 해야 GHA runtime 에 노출돼요 (Phase 2 자동화 전까지).

---

## 개요

도그푸딩 환경 셋업 시 자주 묻는 질문 17개 모음이에요. 본 가이드 ([`도그푸딩 환경 셋업 가이드`](./dogfood-setup.md)) 를 따라가다 막히는 지점별 해결 포인터예요.
