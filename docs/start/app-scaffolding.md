...
이 문서는 새로운 앱 도메인 모듈을 생성하는 `tools/new-app/new-app.sh` 스크립트를 정리해요.

템플릿은 하나의 바이너리로 여러 앱을 호스팅하는 **모듈러 모놀리스** 구조예요. 앱을 추가할 때 Gradle 모듈, AutoConfiguration, 컨트롤러 스켈레톤, Flyway 마이그레이션, DataSource 설정, `.env` 변수, Postgres schema/role 까지 **여러 곳을 동시에** 건드려야 하는데, 이를 수작업으로 반복하면 실수가 쌓여요. `new-app.sh` 는 이 과정을 **한 번에, 멱등하게** 수행해요.

> **한 줄 요약** — 모듈러 모놀리스에 새 앱 도메인을 추가하는 스크립트예요. Gradle 모듈 · Java 컨트롤러 · Flyway 마이그레이션 · `.env` 변수 · Postgres schema/role 을 **한 번에, 멱등하게** 생성해요.
>
> **최소 명령**: `./tools/new-app/new-app.sh <slug> --provision-db` — `--provision-db` 를 붙이면 DB schema/role 까지 자동 생성되므로 **기본으로 붙이길 권장**해요.

---

## 1. 사전 조건 — `bootstrap.sh` 먼저 돌린 상태

`new-app.sh` 는 환경이 이미 부팅된 상태를 가정해요. 처음 파생 레포를 clone 했다면 다음 순서로 진행해요.
...
이 상태까지 갖춰져 있어야 `new-app.sh` 가 DB 에 접속해서 schema/role 을 만들 수 있어요.
...
내부적으로 같은 slug 에서 네 가지 변형이 만들어져 각각 용도에 맞게 사용돼요.
...
앱 모듈은 `core-*-api` 만 의존해요. `core-*-impl` 에 직접 의존하지 않아서 모듈 경계가 지켜져요.
...
**HealthController** 는 `GET /api/apps/<slug>/health` 를 제공해서 빠르게 동작 확인을 할 수 있게 해줘요.
...
**AuthController** 는 `core/core-auth-impl/src/main/java/com/factory/core/auth/impl/controller/AuthController.java` 를 레퍼런스 소스로 삼아 복제돼요. `AuthPort` 에 위임하는 얇은 컨트롤러이고, 경로는 `/api/apps/<slug>/auth/*` 예요.

> `core-auth-impl` 의 `AuthController` 는 `@Import` 되지 않아 런타임 bean 으로 등록되지 않아요. 앱 모듈이 추가되는 순간부터 해당 slug 의 인증 엔드포인트만 노출돼요. 템플릿 상태(앱 0개) 에서는 인증 엔드포인트가 전혀 노출되지 않아요.
...
> 슬러그 모듈의 `AutoConfiguration` 과 `DataSourceConfig` 에는 `@Profile("!test")` 가 함께 붙어 있어요. bootstrap test (Testcontainers 단일 DB) 환경에서는 슬러그 모듈이 비활성화돼서, 슬러그별 schema 가 없는 테스트 환경에서도 부팅이 가능해요.

Spring Boot 가 이를 인식하도록 `AutoConfiguration.imports` 파일도 같이 생성돼요.
...
각 앱은 **독립된 schema + DataSource + Flyway 히스토리**를 가져요. `<SlugPascal>DataSourceConfig` 가 이를 담당해요.
...
V001~V006 은 인증/디바이스 공통 테이블이에요 (`core-auth`, `core-user`, `core-device` 엔티티에 매핑). V007 이상부터는 앱 도메인 테이블을 직접 작성해요 (§7 참조).

마이그레이션 경로가 `db/migration/<slug>/` 처럼 slug 별로 격리되어 있어서, 각 앱 DataSource 의 Flyway 가 자기 디렉토리만 읽어요.

### 3.6 README / settings.gradle / bootstrap.gradle 업데이트

`apps/app-<slug>/README.md` 에 해당 앱의 구조와 템플릿 동기화 방법이 기록돼요. `template-v*` 태그가 있으면 그 버전을 함께 기록해요.
...
bootstrap 은 앱 모듈을 `implementation` 으로 의존해야 `@AutoConfiguration` 이 활성화되고 컨트롤러가 런타임에 노출돼요.

---

## 4. 환경 변수 (`.env`) 주입

`.env` 파일이 없으면 `.env.example` 에서 복사하고, 다음 변수들을 추가해요 (이미 있으면 skip — §6 멱등성 참조).
...
`<host>` 는 수동으로 교체해야 해요 (로컬은 `localhost:5433`, 운영은 Supabase pooler 호스트 등). `--provision-db` 를 지정하면 `_DB_PASSWORD` 는 자동으로 랜덤 생성되어 치환돼요 — §5 참조.
...
`<N>` 은 기존 `APP_STORAGE_MINIO_BUCKETS_*` 변수들의 최대 인덱스 + 1 로 결정돼요. Spring 기동 시 `BucketProvisioner` 가 해당 이름의 버킷을 MinIO 에 자동 생성해요.
...
실제 값 발급 방법은 [`소셜 로그인 설정 가이드`](./social-auth-setup.md) 를 참조하세요.

### 4.4 멱등성 — 이미 있는 키는 skip

`.env` 에 이미 동일한 키가 있으면 덮어쓰지 않고 넘어가요. 같은 명령을 다시 돌려도 기존 값은 안전해요.
...
전체 멱등성 매트릭스(`.env` / Gradle / DB) 는 §6 을 참조하세요.

---

## 5. Postgres Schema / Role Provisioning (`--provision-db`)

앞 §3, §4 가 **코드와 설정 파일** 만 만들었다면, 이 단계는 **실제 DB 에 물리적으로 공간을 마련하는** 작업이에요. `--provision-db` 플래그로 opt-in 하면 스크립트가 `psql` 을 호출해서 `infra/scripts/init-app-schema.sql` 을 실행해요.
...
생성된 비밀번호는 `.env` 의 `<SLUG_UPPER>_DB_PASSWORD=CHANGE_ME` placeholder 에 자동으로 치환돼요.
...
스크립트 내부적으로 `DATABASE_URL` 환경변수가 shell 에 없으면 `.env` 에서 자동 로드해요. Docker 로컬 환경은 결정적이라 사용자 조작이 전혀 필요 없어요.

### 5.3 운영 DB 에 provision 하는 경우

운영 admin credential 은 **`.env` 에 저장하지 않고** shell 에서 일시 export 해요. shell 환경변수가 `.env` 값보다 우선 사용되므로 로컬 기본값을 자연스럽게 덮어써요.
...
`DATABASE_URL` 은 schema/role 을 **생성할 권한이 있는 관리자 credential** 이어야 해요 (앱 role 아님). 로컬에선 docker-compose 의 superuser, 운영에선 Supabase `postgres` 같은 계정이에요. `psql` 이 설치되어 있어야 해요 (`brew install libpq` 또는 `postgresql`).
...
`openssl rand -hex 24` 로 48자 hex 비밀번호를 만들고, SQL 실행 후 `.env` 의 placeholder 를 해당 비밀번호로 치환해요.
...
스크립트는 slug 형식도 방어해요 — Postgres identifier 는 소문자/숫자/밑줄만 허용하므로, 하이픈이 포함된 slug 는 내부적으로 `SLUG_PACKAGE` (하이픈 제거) 로 변환되어 전달돼요. 형식이 어긋나면 `division-by-zero` trick 으로 실행이 중단돼요.

---

## 6. 멱등성

`new-app.sh` 자체는 같은 slug 로 두 번 실행해도 안전하도록 설계되지는 **않았어요** — 앱 모듈 디렉토리가 이미 있으면 즉시 종료해요.
...
반면 **스크립트가 건드리는 개별 요소들**은 각각 멱등해요. 예를 들어 디렉토리만 직접 삭제하고 재실행하면 `.env` / DB schema 는 영향 없이 재사용돼요.
...
재시도 흐름 — 스크립트가 `apps/app-<slug>/` 생성 후 중간에 실패한 경우, 해당 디렉토리만 수동 삭제하고 (`rm -rf apps/app-<slug>`) 재실행하면 나머지 멱등 요소가 skip 되어 안전하게 이어져요. DB schema 는 그대로 두어도 돼요.

---

## 7. 실행 후 남은 수동 작업

스크립트 마지막에 무엇이 **자동으로 처리됐고** 무엇이 **남아있는지** 를 안내해요. 이때 출력되는 내용은 `--provision-db` 플래그를 **붙였느냐에 따라 달라져요**. DB schema 를 자동 생성했느냐 여부가 "남은 작업" 개수에 영향을 주기 때문이에요.
...
스크립트가 DB schema 와 role 까지 만들어 줘서 **남는 작업이 한 가지 줄어들어요**. 실제 스크립트 출력은 다음과 같아요.
...
이 케이스에서는 **DB schema 수동 생성 단계가 출력에 아예 등장하지 않아요.** 이미 DB 에 반영되었기 때문이에요. 로컬 `docker compose` 환경이든 운영 Postgres 든 동일하게 적용되고, `new-app.sh` 가 실행 시점에 `.env` 의 `DATABASE_URL` (또는 shell export) 을 보고 해당 DB 에 적용해요.
...
스크립트는 코드 파일과 `.env` 항목만 준비하고, **DB 는 건드리지 않아요.** 이 경우 출력에는 DB schema 생성이 "남은 작업" 1번으로 추가돼요.
...
이때 schema 를 직접 만들고 싶지 않다면 `./tools/new-app/new-app.sh gymlog --provision-db` 로 **한 번 더 실행** 하면 돼요. 스크립트는 멱등하게 설계되어 있어서 이미 만들어진 코드 파일은 건드리지 않고, DB schema / role 만 추가로 생성해요.

### 7.3 요약 — 사람이 직접 할 일

두 케이스 모두 **공통으로 남는 3가지** 작업이에요.

1. `.env` 의 `<host>` 자리와 소셜 로그인 credential placeholder 를 실제 값으로 교체
2. 도메인 테이블용 `V007__init_domain.sql` 작성 (이 앱의 비즈니스 로직)
3. 커밋

여기에 **케이스 B** 에서는 **"DB schema 수동 생성"** 이 하나 더 추가돼요. DB 를 처음부터 자동으로 만들게 하려면 **케이스 A (`--provision-db` 를 붙이는 쪽)** 를 기본으로 쓰는 것을 권장해요.
...
스크립트 중간에 실패했거나 일부만 생성된 경우에 발생해요.
...
V001~V006 의 체크섬이 맞지 않으면 Flyway 가 거부해요. 공통 마이그레이션 파일을 수정하지 않았다면 원인은 대개 DB 에 남은 이전 실행 흔적이에요. schema 를 drop 하고 재생성해요.
...
`--provision-db` 를 쓰지 않았거나, 스크립트가 sed 치환 전에 실패한 경우예요. `.env` 에서 직접 값을 넣거나, schema 를 drop 한 뒤 `--provision-db` 로 재실행하면 자동 치환돼요.

### 8.4 Postgres identifier 에러 (하이픈 관련)

slug 자체는 하이픈을 허용하지만 Postgres schema/role 이름에는 `SLUG_PACKAGE` (하이픈 제거 버전) 가 사용돼요. `my-app` slug 로 만든 앱의 schema 는 `myapp` 이에요. 수동 psql 명령 실행 시 `APP_SLUG` 환경변수는 반드시 하이픈을 뺀 버전으로 설정해야 해요.
...
- [`소셜 로그인 설정 가이드`](./social-auth-setup.md) — Google/Apple credential 발급
