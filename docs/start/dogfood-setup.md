...
### 무엇을 하나요
- `tools/init-local.sh` (.env / docker) + `tools/init-prod.sh` 1·2회차 (.env.prod 자동 생성 + GitHub Secrets / Variables push)
- `tools/verify-server.sh` 7단계 검증 (init-prod.sh Step 10 에서 자동 호출) — backend health UP / DB / SSH+TS / MinIO / Email / Loki / Alertmanager
...
- `init-local.sh` + `init-prod.sh` 1·2회차 + `verify-server.sh`: ~10분
...
./factory init                             # ← REPO 인자 없음 (= init-local.sh && init-prod.sh)
```

- `init-local.sh` + `init-prod.sh` 둘 다 3-sentinel(rename 완료 / `PROJECT_README_TEMPLATE.md` 부재 / `.env.prod` 부재) 로 공동 작업자 모드를 자동 감지해요.
- init-local 의 Step 2/3 + init-prod 전체 자동 skip → `.env` 생성 + docker compose + verify-local 만 진행해요.
...
- `init-prod.sh` 가 `DB_URL` 시작 부분을 검증해서 잘못 넣으면 즉시 fail 해요

> `DB_PASSWORD` 는 `init-prod.sh` Step 5 에서 placeholder 가 들어가 있을 거예요 (자동 발급 X — 외부 DB 발급 비번 그대로 쓰는 디자인). **Supabase 의 실제 비밀번호로 덮어쓰는 것을 잊지 마세요**.
...
(로컬 docker 환경 셋업이 먼저 필요하면 `bash tools/init-local.sh` 또는 `./factory local init` —
rename / .env / docker compose / verify-local 진행. prod 만 셋업할 거라면 아래로 바로.)

```bash
bash tools/init-prod.sh <owner>/<repo>
# 예: bash tools/init-prod.sh storkspear/server-factory
# 또는 ./factory prod init storkspear/server-factory
# 또는 ./factory init storkspear/server-factory  (= local + prod 순차)
```
...
> **`APP_CREDENTIALS_<SLUG>_*` (소셜 로그인) 추가는 자동 push 만**: 자세한 흐름은 [`FAQ Q17`](./dogfood-faq.md#q17) 를 참조하세요. `init-prod.sh` 는 GitHub Secrets 까지만 자동이고, `config/deploy.yml` + `.kamal/secrets` 의 env.secret 목록 추가는 *현재 수동* 이에요.
...
## 6. `init-prod.sh` 2회차 — Secrets / Variables push + `verify-server.sh`

같은 명령을 재실행해요.

```bash
bash tools/init-prod.sh <owner>/<repo>
# 또는 ./factory prod init <owner>/<repo>
```
...
`init-prod.sh` Step 10 에서 자동 호출돼요. 단독 실행도 가능해요: `<repo-name> prod server-test` (또는 `--skip-deploy` 로 SSH/TS 검증을 생략).
...
`init-local.sh` + `init-prod.sh` (1·2회차 셋업) 와 별개로 **Mac mini 에 trial 환경을 임시로 올렸다가 한 번에 cleanup 하고 싶을 때** 사용해요. GHA permissions / SSH key 등록 / GitHub Secrets / Mac mini 컨테이너 / GHCR 이미지를 일괄 처리합니다.
...
2. 새 키 재발급
3. `.env.prod` 갱신 후 `init-prod.sh` 재실행 → 새 키로 GitHub Secrets 자동 갱신
