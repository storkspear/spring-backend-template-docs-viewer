...
- `tools/verify-server.sh` 6단계 검증 (init-server.sh Step 10 에서 자동 호출) — backend health UP / DB / SSH+TS / MinIO / Email / Loki
...
> **공동 작업자** (이미 셋업된 레포를 fresh clone 받은 두 번째+ 작업자) 는 다른 흐름. → [`FAQ Q12`](./dogfood-faq.md#q12)
