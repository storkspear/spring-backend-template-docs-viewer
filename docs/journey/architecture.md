...
> **독자 대상**: Spring 실무 중급 (Level 2). 이 문서는 하루 안에 전체 구조를 이해하고 특정 모듈을 수정할 수 있도록 안내합니다. Level 0~1 은 [`onboarding.md`](./onboarding.md) 먼저 참고.
...
schema 실제 구조와 마이그레이션 상세는 `infrastructure.md §10` ([`../infra/infrastructure.md`](../infra/infrastructure.md)).
...
Gradle 이 잡지 못하는 패턴 (같은 패키지 이름 규칙, Entity 노출 금지, `*Mapper` 클래스 금지 등) 은 ArchUnit 22 개 규칙 (r1~r22) 으로 빌드 시 검증. 상세: [`../architecture/architecture-rules.md`](../architecture/architecture-rules.md).
...
포맷 상세: [`../api-contract/api-response.md`](../api-contract/api-response.md).
...
이 레이어가 **추출 가능성의 형식적 보증** — `core-auth-impl` 을 HTTP 클라이언트 어댑터로 교체해도 같은 계약 테스트를 통과하면 정상 작동 확정. 상세: [`../testing/contract-testing.md`](../testing/contract-testing.md).
...
상세: [`../testing/testing-strategy.md`](../testing/testing-strategy.md).
...
- [`../STYLE_GUIDE.md`](../STYLE_GUIDE.md) — 문서 작성 규칙
- [`onboarding.md`](./onboarding.md) — 로컬 개발 환경 셋업

### 모듈 / 아키텍처 세부
- [`../architecture/module-dependencies.md`](../architecture/module-dependencies.md) — 의존 방향 규칙 상세
- [`../architecture/architecture-rules.md`](../architecture/architecture-rules.md) — ArchUnit 22 규칙 전체 (r1~r22)
- [`../architecture/multitenant-architecture.md`](../architecture/multitenant-architecture.md) — per-app schema + HikariCP 격리 + appSlug 검증
- [`../architecture/jwt-authentication.md`](../architecture/jwt-authentication.md) — JWT claims · access/refresh · @CurrentUser · BCrypt

### 인프라 / 운영
- [`../infra/infrastructure.md`](../infra/infrastructure.md) — 맥미니 홈서버, Supabase, Cloudflare Tunnel, 블루그린 배포
- [`../infra/ci-cd-flow.md`](../infra/ci-cd-flow.md) — commit → 운영 반영 전체 흐름
- [`../infra/runbook.md`](../infra/runbook.md) — 운영 절차 + 장애 대응

### API 계약
- [`../api-contract/api-response.md`](../api-contract/api-response.md) — 응답 포맷 표준
- [`../api-contract/flutter-backend-integration.md`](../api-contract/flutter-backend-integration.md) — Flutter 앱 연동

### 컨벤션 / 기능 가이드
- [`../conventions/`](../conventions/) — 네이밍, DTO factory, 예외 처리, git-workflow 등
- [`../features/push-notifications.md`](../features/push-notifications.md) — FCM 디바이스 등록 + PushPort
- [`../features/email-verification.md`](../features/email-verification.md) — Resend 이메일 인증 + 비밀번호 재설정
- [`../features/storage.md`](../features/storage.md) — 파일 스토리지 컨벤션
