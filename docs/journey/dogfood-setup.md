...
template (또는 파생 레포) 가 자기 자신을 Mac mini 에 배포해서 한 사이클 검증하기 위한 가이드. **정상 흐름만** 다룸. 에러 만나면 → [`../troubleshooting/dogfood-pitfalls.md`](../troubleshooting/dogfood-pitfalls.md).

> 결정 근거: [`../conventions/decisions-infra.md` I-09 ~ I-14](../conventions/decisions-infra.md)
> 전체 플로우: [`../architecture/ci-cd-flow.md`](../architecture/ci-cd-flow.md)
...
GHCR 에 docker 이미지 push 할 권한이 필요합니다. `GITHUB_TOKEN` 으론 첫 패키지 생성 시 권한이 부족 (알려진 이슈 — [pitfalls #7](../troubleshooting/dogfood-pitfalls.md)) → PAT 사용.
...
- **Custom scopes** 선택, 다음 **2개 scope 모두** 체크 (둘 중 하나 빠지면 403 — [pitfalls #4](../troubleshooting/dogfood-pitfalls.md)):
...
(이 단계 누락 시 GHCR push 403 — [pitfalls #5](../troubleshooting/dogfood-pitfalls.md))
...
**⚠️ 그대로 쓰면 안 됨** — JDBC 형식 아님 ([pitfalls #11](../troubleshooting/dogfood-pitfalls.md)). 다음처럼 분리:
...
내부 Tailscale IP 만으로 검증할 거면 skip. 외부에서 `https://server.<도메인>` 으로 접근 원하면 [`./deployment.md` §2.3 ~ §2.6`](./deployment.md) 참조.
...
자세히: [`../security/key-rotation.md`](../security/key-rotation.md)
...
자세한 rotation 정책: [`../security/key-rotation.md`](../security/key-rotation.md)

---

## 9. 트러블슈팅

에러 만나면 → [`../troubleshooting/dogfood-pitfalls.md`](../troubleshooting/dogfood-pitfalls.md) 의 표에서 에러 메시지로 검색.
...
- [`../architecture/ci-cd-flow.md`](../architecture/ci-cd-flow.md) — commit→배포 전체 다이어그램
- [`../troubleshooting/dogfood-pitfalls.md`](../troubleshooting/dogfood-pitfalls.md) — 11회 시도 함정 모음
- [`./dogfood-faq.md`](./dogfood-faq.md) — 자주 묻는 질문
- [`../security/key-rotation.md`](../security/key-rotation.md) — 키 교체 절차
- [`./deployment.md`](./deployment.md) — 운영 배포 (cloudflared 셋업, observability 등)
- [`./mac-mini-setup.md`](./mac-mini-setup.md) — Mac mini 운영 호스트 셋업
- [`../runbook.md`](../runbook.md) — 평시 배포 / 롤백 / 장애 대응

---

## 📖 책 목차 — Journey 4~6단계

[`../README.md`](../README.md) 의 **4단계 (외부 자격 증명) · 5단계 (테스트) · 6단계 (정리)** 입니다. 한 문서가 세 단계를 통합합니다.
...
**막혔을 때**: [`../troubleshooting/dogfood-pitfalls.md`](../troubleshooting/dogfood-pitfalls.md) / [`./dogfood-faq.md`](./dogfood-faq.md)
**왜 이렇게?**: [`../conventions/decisions-infra.md`](../conventions/decisions-infra.md) — I-09 (Kamal 선택) / I-10 (GHCR PAT) / I-12 (workflow_run 게이트) / I-14 (Tailscale OAuth scope)
