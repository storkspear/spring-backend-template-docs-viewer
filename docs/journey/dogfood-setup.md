...
GHCR 에 docker 이미지 push 할 권한이 필요합니다. `GITHUB_TOKEN` 으론 첫 패키지 생성 시 권한이 부족 (알려진 이슈 — [pitfalls #7](../reference/dogfood-pitfalls.md)) → PAT 사용.
...
- **Custom scopes** 선택, 다음 **2개 scope 모두** 체크 (둘 중 하나 빠지면 403 — [pitfalls #4](../reference/dogfood-pitfalls.md)):
...
(이 단계 누락 시 GHCR push 403 — [pitfalls #5](../reference/dogfood-pitfalls.md))
...
**⚠️ 그대로 쓰면 안 됨** — JDBC 형식 아님 ([pitfalls #11](../reference/dogfood-pitfalls.md)). 다음처럼 분리:
