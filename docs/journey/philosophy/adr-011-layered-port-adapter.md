...
자세한 내용은 [ADR-013 (앱별 인증 엔드포인트)](./legacy-pending-rewrite.md) 참조.
...
- 각 앱 모듈이 자기 `<Slug>AuthController` 를 가지며 경로는 `/api/apps/<slug>/auth/*` — [ADR-013](./legacy-pending-rewrite.md) 에서 상세
