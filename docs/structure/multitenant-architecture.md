...
### 슬러그별 자격 derive (`<SLUG>_DB_URL` 비우기 패턴)

`AbstractAppDataSourceConfig` 는 `<SLUG>_DB_URL` 이 비어있을 때 core 의 `${DB_URL}` 에서 `currentSchema=<slug>` 부분만 슬러그로 자동 교체해요. USER 와 PASSWORD 도 core 자격을 그대로 재사용합니다.
