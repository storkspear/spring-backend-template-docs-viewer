...
레포 루트 `README.md` 의 30분 QuickStart 만으로도 첫 기동은 돼요. 이 책은 그 이후, 레포의 정체와 사용 흐름을 차근차근 이해하고 싶을 때 읽는 안내서예요.
...
> 💡 **아직 "이 레포를 쓸지 말지" 결정 전** 이면 먼저 [`Level 0 진입점 (getting-started)`](./getting-started.md) (3~10분) 를 읽으세요. 코드를 돌리지 않고도 레포의 정체와 구조 큰 그림이 잡혀요.
...
이 레포의 **정체** 를 이해해요. 어떤 종류의 프로젝트이고, 왜 이렇게 설계됐는지 큰 그림을 잡아요.
...
2. [`Architecture Reference`](../structure/architecture.md) 의 **§ 전체 구성 요약** 한 섹션만 읽으세요. 모듈 4종류 (`common/` · `core/` · `apps/` · `bootstrap`) 와 기술 스택의 한눈 요약이 있어요.

여기까지 읽으면 "이 레포가 뭘 하려는 도구인지" 감이 잡혀요. 나머지 ADR (총 32개, 테마 2~8) 은 나중에 해당 영역이 궁금해질 때 돌아오면 돼요. *어떤 L2 문서가 어느 ADR 의 결과인지* 는 [`philosophy/README.md`](../philosophy/README.md) 끝의 **L2 ↔ L3 매핑** 표를 보면 빠르게 찾을 수 있어요.

---

## 2. 어떻게 써? — 로컬 개발 (1시간)

본인 노트북에서 dev 환경을 띄우고 Spring 을 직접 돌려 봐요.
...
template 은 비즈니스 로직 없이 뼈대만 가지고 있어요. 실제로 쓰려면 **앱 모듈** 을 하나 추가해야 해요.
...
이 명령은 `apps/app-<slug>/` 디렉터리를 만들고, Postgres 에 앱 전용 schema + role 을 자동 생성해요. 자세한 동작과 생성되는 파일 목록은 위 §5 에 표로 정리돼 있어요.
...
운영 배포로 넘어가려면 외부 서비스의 자격 증명을 발급받아야 해요. 어디서 어떻게 받는지가 막히는 지점이라, 두 문서로 나뉘어 안내해요.
...
각 발급 후 `.env` 의 `APP_CREDENTIALS_<SLUG>_*` 변수에 채워 넣어요.
...
각 항목은 화면 캡처 없이도 따라갈 수 있게 클릭 경로 + 주의사항 (잘못 발급되는 함정 포함) 까지 적혀 있어요.

---

## 5. 테스트 어떻게? — 도그푸딩 자동 검증 (자동)

발급 받은 값으로 template 자체가 운영 환경에 올라가는지 한 사이클 검증해요.

읽을 문서:

- [`도그푸딩 환경 셋업 가이드`](../start/dogfood-setup.md) **§4 ~ §6** (작성 + 실행 + 동작 확인) — 정상 흐름.
- [`도그푸딩 walkthrough`](../start/dogfood-walkthrough.md) — *시간 순 narrative + 정착된 패턴*. 함정마다 왜 이런 영구 fix 가 정착됐는지 흐름으로 풀어 드려요.
- 막히면 [`도그푸딩 함정 모음 (사고 실록)`](../start/dogfood-pitfalls.md) — 12 함정 reference.
...
`setup.sh` 가 GitHub Actions Variables / Secrets 일괄 등록 + GHA 용 SSH 키 발급 + DEPLOY_ENABLED 토글 + 자동 배포 trigger 까지 한 번에 처리해요.
...
검증이 끝났으면 깨끗하게 정리하고 template 순수 상태로 돌려요.
...
외부 서비스 (PAT / Tailscale OAuth) 의 키 자체는 **본인이 직접 폐기** 해야 해요. 절차는 [`키 교체 절차 (Key Rotation)`](../production/setup/key-rotation.md) 에 있어요.
...
핵심 흐름은 도그푸딩과 거의 동일해요 (5~6단계). 차이는 다음 두 가지예요.

- 파생 레포는 본인 도메인 / 본인 인프라 값으로 채워요.
...
문서는 독자 Level 별로 9 그룹으로 묶여 있어요. 처음 방문이라면 위에서 아래로, 숙련되면 관심 그룹만 펼쳐 보세요.
...
| 4-stage secret chain | [`secret chain 4-stage 통합 가이드`](../production/setup/secret-chain-4stage.md) | 4 곳 매핑 + 체크리스트 |
...
7단계까지 한 번 흐르고 나면 template 의 전체 사용 흐름이 머릿속에 잡혀요. 그 다음은 **본인 프로젝트의 도메인을 만들어 나가는 것** 이 자연스러운 다음 단계예요.

진행하면서 막히는 부분이 있으면 위 "깊이 있는 참조" 의 해당 문서를 펼쳐 보세요. 모든 문서는 서로 연결돼 있고, 어디로 가야 할지 막힌다면 이 책 목차로 다시 돌아오면 돼요.
