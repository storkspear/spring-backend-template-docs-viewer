...
이 레포의 문서를 읽다가 **"이게 뭐지?"** 싶으면 여기서 먼저 찾아보세요. 엄밀한 정의보다 **직관적 이해 우선**. 라이브러리 · 버전 · 외부 서비스 인벤토리는 [`Environment`](./environment.md) 를 참고.
...
| **Spring Framework** | Spring Boot 의 기반. DI · AOP · MVC 등의 뼈대. Boot 은 이걸 **기본값 잔뜩 깔아놓은 버전** |
...
| **jjwt** | Java JWT 라이브러리. 이 레포가 사용하는 것 |
...
| **springdoc-openapi** | Spring Boot 자동 OpenAPI 문서 생성. `/v3/api-docs` · Swagger UI 제공. 버전 · 경로는 `build.gradle.kts`, `application.yml` 의 springdoc 설정을 기준으로 확인 |
