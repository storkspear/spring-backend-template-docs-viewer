...
이 문서는 앱 공장 모델에서 발생할 수 있는 엣지케이스를 분류하고, 각각의 영향과 해결 방법을 제시해요.

**분류 기준**:
- 🔴 **대형 사고** — 한 번 발생하면 데이터 유출, 매출 손실, 법적 문제. 반드시 사전 차단해야 합니다.
- 🟡 **중간 리스크** — 서비스 장애나 UX 혼란. 대응 방안이 필요합니다.
- 🟢 **낮은 리스크** — 불편하지만 서비스에 심각한 영향이 없어요. 인지만 하면 됩니다.
...
**상태:** Phase 0 리팩토링에 포함됩니다. 필수예요.

---

### 🔴 1-2. JWT 비밀키 유출 — 모든 앱의 토큰 위조 가능

**시나리오:** JWT HS256 비밀키가 git 커밋, 로그 파일, 환경변수 노출 등으로 유출됐어요. 공격자가 임의의 userId + appSlug 로 JWT 를 위조해 어떤 앱의 어떤 유저로든 접근할 수 있어요.

**영향:** 전체 서비스의 모든 유저 데이터 탈취 가능. 최악의 보안 사고.

**해결:**
- 비밀키는 **환경변수로만** 제공해요. `.env` 는 `.gitignore` 에 포함됩니다.
...
**상태:** 기본 방어 (키 길이 검증, .gitignore) 는 Phase 0 에 포함돼 있어요. 이중 키 검증은 Phase 1 이에요.
...
- **SHA-256 해시 저장**: DB 에 raw token 을 저장하지 않아요. DB 유출 시에도 토큰 위조가 불가능해요.
...
**상태:** 기본 방어 (rotation + 탈취 감지) 는 Phase 0 에 포함돼 있어요. fingerprint 바인딩은 Phase 1 이에요.
...
**상태:** (향후 PasswordResetService 구현 시 반영)
...
**시나리오:** FCM service account JSON 파일이 git 에 커밋되거나 유출됐어요. 공격자가 모든 디바이스에 스팸 푸시를 발송할 수 있어요.
...
- 이 문서를 향후 담당자가 참조할 수 있도록 `SecurityConfig` javadoc 에 경고를 포함합니다.

---

### 🔴 1-9. Apple Sign In — 클라이언트 제출 email 신뢰 시 계정 탈취

**시나리오:** Apple identity token 의 `email` claim 과 별도로 클라이언트가 `request.email` 을 전송할 수 있다. 만약 서버가 클라이언트가 보낸 `request.email` 을 Apple 이 RS256 으로 서명한 `tokenEmail` 보다 우선 사용하면, 공격자가 자기 Apple 계정의 identity token 으로 피해자의 이메일 주소를 가진 계정을 생성할 수 있습니다.
...
**시나리오:** 잘못된 SQL 이 마이그레이션 파일에 포함된 상태예요. Spring Boot 기동 시 Flyway 가 마이그레이션을 실행 → SQL 에러 → **앱 전체 부팅 실패** → 모든 앱이 다운돼요.
...
**시나리오:** 유저가 탈퇴 (soft delete) 한 상태예요. 30 일 후 hard delete 를 해야 하는데 (개인정보보호법 / GDPR 요구), 스케줄러가 없어서 데이터가 영구 보존돼요. 법적 요구를 위반하게 됩니다.
...
- 이 시나리오는 근본적으로 **유저가 여러 앱을 쓸 때만** 발생하며, 인디 앱 공장에서 한 유저가 여러 앱을 동시에 쓸 확률은 매우 낮습니다.
...
**시나리오:** Supabase 계정에 문제 발생 (결제 실패, ToS 위반, 계정 해킹 등). 모든 앱의 DB 접근이 차단됩니다.
...
- **이중 백업**: Supabase 자체 일일 백업 + NAS `pg_dump`. 두 개 모두 실패할 확률은 극히 낮습니다.
...
**시나리오:** 월 3,000 통 (일 100 통) 한도가 소진돼요. 이후 가입한 유저는 이메일 인증 메일을 받지 못하고, 비밀번호 재설정도 불가능해져요.
...
**해결:**
- **앱별 HikariCP 풀 격리** (이미 설계됨): `maximum-pool-size` 를 앱별로 설정. 한 앱의 고갈이 다른 앱에 전파되지 않습니다.
...
| 요구사항 | 현재 상태 | 해결 |
|---|---|---|
| **Sign in with Apple** (소셜 로그인 제공 시 필수) | (향후 구현 예정) | Apple 로그인 반드시 포함 |
| **계정 삭제 기능** (2022 년 6 월부터 필수) | (향후 WithdrawService 구현 예정) | Settings 화면에 "계정 삭제" 버튼 필수 |
...
**시나리오:** Google Play Console 에서 "데이터 안전" 섹션을 작성하지 않으면 앱 출시 불가. 앱이 수집하는 데이터 종류, 목적, 공유 여부를 신고해야 합니다.
...
**해결:** 그대로 수용해요. 각 앱의 유저 수가 적고 테이블 20 벌의 오버헤드는 무시할 수 있는 수준이에요. 격리의 대가로 받아들입니다.
