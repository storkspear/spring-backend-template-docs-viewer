...
이 디렉토리는 `template-spring` 및 이를 파생한 모든 레포의 **코딩 규약** 을 담아요.

규약은 취향이 아니라 **일관성을 위한 계약** 이에요. 혼자 작업하더라도, 6개월 뒤의 나 자신이 과거 코드를 이해할 수 있어야 해요. 그리고 파생 레포가 여러 개 생겼을 때 각 레포의 코드 스타일이 비슷해야 cherry-pick backport 가 매끄러워요.

---

## 개요

이 디렉토리는 `template-spring` 및 파생 레포의 **코드 작성 규약** 을 모아요. 네이밍 · 예외 처리 · DTO 패턴 · git workflow 등 실무 코드에 직접 적용되는 규칙들이에요.
...
- 시스템 구조 → [`structure/`](../structure/) — Module Dependencies · Architecture Rules (ArchUnit) · Multitenant · JWT
- API 계약 → [`api-and-functional/api/`](../api-and-functional/api/) — API Response · JSON Contract · Versioning · Flutter Integration
- 기능 가이드 → [`api-and-functional/functional/`](../api-and-functional/functional/) — Push · Email · Observability · Rate Limiting · Storage · Migration · Seed Data
- 테스팅 → [`production/test/`](../production/test/) — Contract Testing · Testing Strategy
...
1. **동작하는 코드** 가 이상적인 규약보다 우선이에요. 규약을 지키기 위해 테스트가 실패하거나 런타임 동작이 깨지면 규약이 틀린 거예요.
2. **이 문서에 명시된 규약** 이 개인 취향보다 우선이에요.
3. **프로젝트 내 기존 패턴** 이 새 패턴보다 우선이에요. 기존 코드와 일관성 있게 따라가고, 필요하면 문서를 업데이트한 후에 일괄 리팩토링해요.
...
사람의 의지로 지키는 규약은 2~3주 안에 무너져요. 기계가 막아주도록 만드는 것이 유일한 지속 가능한 방법이에요.

---

## 모듈 README 유지 규칙

각 모듈 디렉토리(`common/*`, `core/*`, `bootstrap`, `apps/*`) 에는 `README.md` 가 있습니다. 코드를 변경할 때 다음 중 하나라도 해당되면 **같은 커밋에서** 해당 모듈의 `README.md` 를 함께 업데이트해요.
...
별도 "docs 업데이트" 커밋으로 분리하지 않고, 코드 변경과 같은 커밋에 포함해요. 이렇게 해야 코드와 문서가 어긋나지 않습니다.

---

## 기여

규약은 고정되어 있지 않습니다. 개선 의견이 있으면 해당 문서에 직접 수정을 제안하거나, 기존 규약이 실제 코드 작성에서 걸림돌이 되면 즉시 재평가해요. 다만 **"이 결정의 이유가 여전히 유효한가"** 를 먼저 확인한 후에 바꿔요.
