...
이 디렉토리는 `template-spring` 및 이를 파생한 모든 레포의 **코딩 규약** 을 담습니다.
...
이 디렉토리는 `template-spring` 및 파생 레포의 **코드 작성 규약** 을 모읍니다. 네이밍 · 예외 처리 · DTO 패턴 · git workflow 등 실무 코드에 직접 적용되는 규칙들.
