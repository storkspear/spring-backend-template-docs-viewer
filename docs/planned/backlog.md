...
- [ ] [DX] `tools/dev-cleanup.sh` 의 workspace 부재 = "이미 정리됨" 판단 bug — `~/workspace/${KAMAL_SERVICE_NAME}/` 디렉토리 부재만 보고 Mac mini kamal app 정리 skip 하지만, 실제로는 컨테이너 + 이미지 + kamal-proxy routing entry 가 별도로 살아있을 수 있음. 도그푸딩 cycle (example-backend dev deploy/cleanup) 에서 workspace 디렉토리 없는데 컨테이너 4개 + 이미지 2개 + kamal-proxy routing 남아 수동 정리 필요. 보강: workspace 디렉토리 부재 → workspace archive 만 skip, `docker stop/rm` + `docker rmi` + `kamal-proxy remove` 는 label 기준 (`service=<svc>-dev`) 으로 별도 수행 (2026-05-18)
- [ ] [DX] `BucketProvisioner` endpoint reach 실패 silent skip 진단성 강화 — MinIO endpoint 가 LAN 분리/방화벽 등으로 reach 안 될 때 부팅은 통과하지만 bucket auto-create 가 silent skip. 도그푸딩 cycle 에서 NAS docker container 의 MinIO 가 LAN 외부 unreachable 인데 Spring readiness UP 으로 인지 어려움. 보강: ①startup log 에 명시 warn (`BucketProvisioner: endpoint reach failed, bucket auto-create skipped`). ②strict mode 옵션 — endpoint reach 실패 시 startup fail (옵션 키 e.g. `app.storage.minio.strict-startup=true`) (2026-05-18)
- [ ] [DX] `core-storage-impl` 의 `@ConditionalOnProperty(name = "app.storage.minio.endpoint", matchIfMissing = false)` 빈 값 edge case — Spring 의 `@ConditionalOnProperty` 가 property 존재 기준이라 `APP_STORAGE_MINIO_ENDPOINT=` (빈 값) 도 *exists* 로 판단 → MinioClient bean 생성 시도 → `endpoint must be a non-empty string` 으로 부팅 fail. `.env.dev.example:154` 가이드 ("비어있으면 InMemoryStorageAdapter fallback") 와 실 동작 불일치. fix: `@ConditionalOnExpression("'${app.storage.minio.endpoint:}' != ''")` 로 교체 → empty string 도 missing 처럼 처리. 도그푸딩 cycle 에서 dev deploy 시 fail-fast 로 발견 (2026-05-18)
- [ ] [DX] `init-dev.sh` 가 dev MinIO bucket 에 `dev-` prefix 자동 부여 또는 강제 검증 — dev/prod 가 같은 MinIO 인스턴스 공유 디자인이라 bucket 이름 충돌 시 사고 위험 (dev force-clear 가 prod bucket 까지 삭제). `init-dev.sh` 에서 사용자 입력 BUCKETS_0 에 자동 `dev-` prefix 부여 또는 *prefix 없으면 재입력 요구*. 보조: `dev-force-clear.sh` 가 *prod prefix 패턴* (e.g. `prefix 가 dev- 아님*) 인 bucket 발견 시 차단 (2026-05-18)
