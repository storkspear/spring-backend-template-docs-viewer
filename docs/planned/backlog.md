...
- [x] ~~multi-app prod env 자동 derive 패턴~~ — `common-persistence/AbstractAppDataSourceConfig.deriveSlugUrlOrEmpty` 에 구현 완료. `<SLUG>_JDBC_DB_URL` 비우면 `${JDBC_DB_URL}` 의 `currentSchema=<slug>` 자동 교체, USER/PASSWORD 는 core 그대로 재사용 (2026-05-03)
