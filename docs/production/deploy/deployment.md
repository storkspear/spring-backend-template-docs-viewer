...
**GHCR 토큰**: ⚠️ `secrets.GITHUB_TOKEN` 자동 주입은 *push 권한 한계* 가 있어 본 template 의 deploy 흐름에는 부족합니다 ([`decisions-infra.md I-10`](./decisions-infra.md) — *GHCR push 용 PAT 필수*, [`dogfood-pitfalls.md #7`](../../start/dogfood-pitfalls.md) 의 함정 사례). Classic PAT (`repo` + `write:packages` scope) 를 발급해 `GHCR_TOKEN` secret 으로 등록해야 합니다. `init-prod.sh` 가 `.env.prod` 의 `GHCR_TOKEN` 을 GitHub Secrets 로 자동 push 하므로, 운영자는 *발급 + .env.prod 채우기* 만 하면 끝.
...
1. `.env.dev` 채우기 — `.env.dev.example` 참고. REQUIRED: `BASE_DOMAIN`, `SUBDOMAIN`, `DB_URL` (별도 Supabase dev 계정), `DB_USER`, `DB_PASSWORD`, `APP_STORAGE_MINIO_BUCKETS_0` (dev 전용 bucket — 같은 MinIO 인스턴스).
   `.env.dev` 안의 키 이름은 `.env.prod` 와 동일 (suffix 없음). `init-dev.sh` 가 GitHub Secrets/Variables 로 push 할 때만 `_DEV` suffix 자동 부여.
   공유 인프라 자격 (Cloudflare / TS_OAUTH / SSH / GHCR / DEPLOY_HOST) 은 `.env.dev` 가 비어있으면 `.env.prod` 에서 자동 fallback.
