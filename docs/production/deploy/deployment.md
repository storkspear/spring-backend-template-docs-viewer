...
```bash
# 파생레포 로컬 clone 후
export PSQL_DB_URL='postgresql://postgres:<pw>@<supabase-host>:5432/postgres'  # Supabase 관리자 credential
...
```bash
JDBC_DB_URL=postgresql://postgres.<ref>:<pw>@aws-1-<region>.pooler.supabase.com:6543/postgres?pgbouncer=true
...
gh secret set JDBC_DB_URL --body 'postgresql://...'
