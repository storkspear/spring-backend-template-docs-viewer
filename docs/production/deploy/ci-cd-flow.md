...
**`git commit` → 사용자에게 노출까지 약 15~20분, billed 8분이에요.**

CI workflow 가 jar 를 만들면 deploy workflow 가 `workflow_run` 으로 자동 트리거됩니다. jar 를 받아서 docker 이미지로 패키징하고 GHCR 에 push 한 뒤 Mac mini 의 kamal 이 swap 합니다. **gradle build 는 1회만** 진행돼요 (CI 가 만든 jar 를 deploy 가 재사용 — 중복 빌드 제거 [`I-12`](./decisions-infra.md)).

> 💡 **push 전 검증** — `factory ci-test` 명령이 GHA 의 5-stage (spotless / build / docs-contract / docs-unit / gitleaks) 를 로컬에서 동일하게 실행해서 push 전에 *사전 통과* 를 보장해요. 자세한 흐름은 [`CLI 가이드 §3 CI 검증`](../../start/cli-guide.md) 을 참조하세요.
...
**husky `commit-msg` hook** 이 다음을 검사합니다.

- `Co-Authored-By: Claude` 라인 정규식 검사 → 매치 시 `exit 1` 로 거절
- `npx commitlint --edit` → conventional commits 룰 검사

→ 위반 시 commit 자체가 거절됩니다 (`.git/objects` 생성 전).

### PHASE 2 — feature 브랜치 push

`ci.yml` 과 `docs-check.yml` 이 `on: push: branches: ['**']` 라서 모든 브랜치 push 시에 동작합니다.

**이 단계에서는 jar artifact 업로드가 일어나지 않아요** — `if: github.event_name == 'push' && github.ref == 'refs/heads/main'` 조건이라 main push 시에만 업로드됩니다.
...
⚠️ **불가역** — 이후 fail 이 나도 자동 revert 가 되지 않아요.

### PHASE 5 — Post-merge CI

`ci.yml` 이 main 에서 다시 실행됩니다. 이번엔 jar artifact 업로드 step 이 동작해요.
...
artifact retention 이 1일이라 storage 를 거의 차지하지 않습니다.
...
kamal-proxy 가 80/443 포트를 listen 합니다. host header 로 라우팅돼요.

- `Host: server.<도메인>` → spring 컨테이너 :8080
- 다른 host → 404

cloudflared tunnel 이 외부 도메인을 Mac mini :80 으로 outbound 연결해요. cloudflared 가 안 떠 있으면 외부 접근이 불가하고 Tailscale IP 로만 접근할 수 있어요.
...
| 12 | Gradle 빌드 | JDK 26 class file major 70 | ✅ init-server.sh prereq (`21 ≤ JAVA < 26` 거부) |

→ **9개는 자동화로 영구 회피, 3개는 사람 손 (외부 서비스 발급) — 가이드가 보완**.
