...
2) 각 슬러그 schema 의 V001~V0NN 순차 적용
      │   <slug>_JDBC_DB_URL 별 datasource 마다 1회씩
...
psql $JDBC_DB_URL -c "SELECT version, description, success, installed_on
...
1. `.env.prod` 에서 JDBC_DB_URL / DB_USER / DB_PASSWORD 로드
...
psql $JDBC_DB_URL -c "SELECT * FROM <schema>.flyway_schema_history ORDER BY installed_rank;"

# 실패한 entry 제거 (정정 V스크립트 적용 전)
psql $JDBC_DB_URL -c "DELETE FROM <schema>.flyway_schema_history WHERE success = false;"

# Advisory lock 강제 해제 (lock 가진 PID 죽이기)
psql $JDBC_DB_URL -c "SELECT pid, pg_terminate_backend(pid) FROM pg_locks WHERE locktype = 'advisory';"
