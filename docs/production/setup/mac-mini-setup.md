...
> - 평시 운영 / 장애 대응: [`운영 런북 (Runbook)`](../deploy/runbook.md)
> - 인프라 결정 카드 (Supabase, MinIO, 맥미니, Tunnel, 관측성, Kamal): [`인프라 결정 기록 (Decisions — Infrastructure)`](../deploy/decisions-infra.md)
...
홈 공인 IP 를 공개 인터넷에 노출하지 않고 외부 접근 허용. TLS/WAF/DDoS 방어는 Cloudflare 엣지에 위임 (Mac mini 부담 ↓). Free plan 이라 비용 0. 대안 (DDNS+포트포워딩 / Tailscale Funnel / nginx+LE) 는 [`decisions-infra.md I-05`](../deploy/decisions-infra.md) 참조.
...
파생레포 하나 안에 modular monolith 로 N 개 앱 모듈을 둡니다. "앱 1 개만 고쳐도 전체 재배포 필요" 구조라 재시작 downtime 이 N 배로 증폭돼요. 1 개 앱 시점에서 B/G 를 셋업해 두면 N 이 증가해도 복잡도가 0 이에요.
...
Template 성격 유지. 코드는 HikariCP + 표준 JDBC 만 사용 — Supabase Realtime/RLS/auth 같은 API 미의존. 파생레포 소유자가 자기 인프라 (AWS RDS 재사용 등) 가 있다면 JDBC_DB_URL 만 바꾸면 된다. Template 관리자 default 는 Supabase (Seoul region, Free tier, Supavisor pooler).

### 7.10 왜 actuator 를 app port 와 공유 (별도 management port 아님)?
원래 `management.server.port: 8090` 으로 분리했지만 Kamal 2.11 의 `proxy.healthcheck` 스키마가 `port:` 키를 받지 않아요. kamal-proxy healthcheck 가 `/actuator/health/liveness` 를 app port 에서 hit 하도록 단일 포트로 통합했어요. Exposure 는 `health / info / prometheus` 만 허용해 민감 엔드포인트를 차단합니다. 더 엄격한 격리가 필요해지면 별도 port + kamal-proxy healthcheck 를 main-port 의 가벼운 엔드포인트로 교체할 수 있어요.
...
자세한 동작은 [`운영 배포 가이드 (파생레포 onboarding)`](../deploy/deployment.md) + [`운영 런북 (Runbook)`](../deploy/runbook.md) 참조.
...
- `JDBC_DB_URL` — Spring 기동 시 `spring.datasource.url` 로 바인딩
- `DB_USER`, `DB_PASSWORD`
- `PSQL_DB_URL` (admin) — `new-app.sh --provision-db` 가 사용. **운영 DB admin credential 은 `.env` 에 저장 금지** — 실행 직전 shell export 로만 주입. (로컬 docker URL 만 `.env.example` 기본값으로 포함됨)

### 15.3 현재 schema 상태 (2026-04-20)
- `core` schema: **존재하지 않음** (도그푸딩 cleanup 에서 DROP CASCADE)
- 파생레포 첫 기동 시 Flyway 가 V001/V002/V003/V005/V006/V007/V008 자동 적용해 core schema 재생성 (V004 reserved)
...
자세한 2-tier bucket 규약: [`오브젝트 스토리지 규약`](../../api-and-functional/functional/storage.md).
...
- `JDBC_DB_URL`, `DB_USER`, `DB_PASSWORD`, `JWT_SECRET`, `APP_DOMAIN`
...
- DuckDNS cron
- Supabase `core` schema (DROP CASCADE — core schema 의 모든 테이블)
...
- [ ] 로컬 clone + 운영 환경 셋업: `cp .env.prod.example .env.prod` → REQUIRED 값 채움 (BASE_DOMAIN / SUBDOMAIN / CLOUDFLARE_API_TOKEN / JDBC_DB_URL / DB_USER / GHCR_TOKEN / SSH_PRIVATE_KEY 등). 로컬 dev 분리 사용 시 `cp .env.example .env` 별도
- [ ] Mac mini SSH:
  - [ ] `ssh-keygen -t ed25519 -f ~/.ssh/gha_deploy -C "gha-deploy@<파생레포>" -N ""`
  - [ ] `cat ~/.ssh/gha_deploy.pub >> ~/.ssh/authorized_keys`
...
- [`운영 배포 가이드 (파생레포 onboarding)`](../deploy/deployment.md) — 파생레포 onboarding (최초 1회)
- [`운영 런북 (Runbook)`](../deploy/runbook.md) — 평시 운영 / 롤백 / 장애 대응
- [`운영 모니터링 셋업 가이드`](./monitoring-setup.md) — 관측성 스택 상세
- [`인프라 (Infrastructure)`](../deploy/infrastructure.md) — 전체 인프라 개요
- [`인프라 결정 기록 (Decisions — Infrastructure)`](../deploy/decisions-infra.md) — 결정 카드 I-01 ~ I-09
- [`Repository Philosophy — 책 안내`](../../philosophy/README.md) — 36 ADR 인덱스 (특히 [`ADR-007`](../../philosophy/adr-007-solo-friendly-operations.md))
