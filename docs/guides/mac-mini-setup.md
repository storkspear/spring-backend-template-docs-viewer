...
>
> 관련 문서:
> - 운영 배포 절차: [`deployment.md`](./deployment.md)
> - 평시 운영 / 장애 대응: [`../runbook.md`](../runbook.md)
> - 인프라 결정 카드 (Supabase, MinIO, 맥미니, Tunnel, 관측성, Kamal): [`../conventions/decisions-infra.md`](../conventions/decisions-infra.md)
> - 전체 인프라 개요: [`../infrastructure.md`](../infrastructure.md)
...
**철학 요약** (자세한 근거는 [`../philosophy.md`](../philosophy.md) / [`../conventions/decisions-infra.md`](../conventions/decisions-infra.md)):
...
│                                                                │
│  SSH authorized_keys:                                            │
│    ├─ hexator0205@gmail.com  (Claude/관리자 접근)                │
...
│      Policy: Allow emails [developerhexator@gmail.com]            │
...
홈 공인 IP 를 공개 인터넷에 노출하지 않고 외부 접근 허용. TLS/WAF/DDoS 방어는 Cloudflare 엣지에 위임 (Mac mini 부담 ↓). Free plan 이라 비용 0. 대안 (DDNS+포트포워딩 / Tailscale Funnel / nginx+LE) 는 [decisions-infra.md I-05](../conventions/decisions-infra.md) 참조.

### 7.3 왜 Docker + Kamal?
- **Docker**: 이미지 = 불변 배포 단위. 버전 롤백이 이미지 tag 하나로 해결.
- **Kamal**: 단일 호스트 blue/green 스왑을 검증된 툴로 운영. 커스텀 bash 재구현보다 신뢰.
- 대안 검토 기록: [decisions-infra.md I-09](../conventions/decisions-infra.md).
...
# GUI 에서 로그인 (hexator0205@gmail.com 계정)
...
- **Policy**:
  - Name: `Admin only`
  - Action: `Allow`
  - Include → Emails → `developerhexator@gmail.com`
...
자세한 동작은 [`deployment.md`](./deployment.md) + [`runbook.md`](../runbook.md) 참조.
...
- [`deployment.md`](./deployment.md) — 파생레포 onboarding (최초 1회)
- [`../runbook.md`](../runbook.md) — 평시 운영 / 롤백 / 장애 대응
- [`monitoring-setup.md`](./monitoring-setup.md) — 관측성 스택 상세
- [`../infrastructure.md`](../infrastructure.md) — 전체 인프라 개요
- [`../conventions/decisions-infra.md`](../conventions/decisions-infra.md) — 결정 카드 I-01 ~ I-09
- [`../philosophy.md`](../philosophy.md) — 철학 / 결정 근거
