...
**Status**: Accepted. `core-*-impl` 6 개 모듈 모두 동일한 레이어 구조를 따릅니다. ArchUnit 규칙 r13, r14, r15, r17, r21 이 클래스 위치를 강제해요.
...
이 질문을 해결하지 못하면 운영에서 세 가지 부담이 곧바로 누적되기 시작해요. 개발자마다 자기 스타일로 코드를 배치할 가능성이 생겨 *일관성이 깨지고*, 비슷한 기능을 찾을 때마다 *매번 다른 위치를 뒤져야 해서 탐색 비용이 올라가며*, 리팩토링 시점이 오면 *"이 클래스가 어디로 가야 하지?" 같은 고민이 반복* 됩니다. 한 명의 솔로 운영자가 N 개 앱을 다루는 환경에서는 이 세 부담이 *시간 소모* 로 직접 환산돼요.

| 답되지 않을 때 부담 | 결과 |
|---|---|
| 개발자별 코드 배치 자유 | 일관성이 깨져 모듈마다 구조가 다름 |
...
- **단점**:
  - 기능 하나 수정에 **3 개 모듈을 건드려야 해요**. 작은 변경도 multi-module 수정이 되어버려요.
  - 도메인 코드가 흩어져서 **전체 파악이 어려움** — "이 앱의 기능 전체" 를 보려면 3개 모듈을 왔다 갔다.
  - Gradle 모듈 수가 폭발적으로 증가 (10 도메인 × 2 레이어 = 20 모듈 + 기존 common/bootstrap).
...
- **장점**: 기능 단위로 파일이 모여 있어요. 한 feature 수정 시 한 패키지만 건드리면 돼요.
- **단점**:
  - **각 모듈이 5~10 클래스 수준** 에서는 feature 분리의 이득이 없어요. 오히려 패키지 수만 많아져서 탐색이 어려워져요.
...
- **장점**:
  ...
  - **기능 수정이 한 모듈 안에서 완결돼요** — Gradle 모듈 여러 개를 건드릴 필요가 없어요.
  - **Best Practice 와 정렬** — Spring 공식 문서, Baeldung, 많은 오픈소스 예제가 같은 구조를 따라요.
  - `@Service`, `@Repository` 같은 Spring 스테레오타입을 **자연스럽게** 쓸 수 있어요.
- **단점**:
  - 레이어 간 의존이 **Gradle 수준으로 강제되지 않아요** — controller 가 repository 를 직접 참조해도 컴파일이 성공해요.
- **완화**: ArchUnit 규칙 r13~r17 이 "어느 클래스가 어느 패키지에 있어야 하는지" 를 바이트코드 수준에서 검증해요. 패키지 위치로 레이어 위반을 간접 차단합니다.
...
**친숙한 구조** — Spring Boot 를 한 번이라도 해본 개발자라면 `controller`, `service`, `repository` 를 바로 이해해요. 새로운 DDD 용어나 커스텀 레이어를 배울 필요가 없어요.

**기능 수정이 한 모듈에서 완결** — "인증에 새 엔드포인트 추가" 시 같은 `core-auth-impl/` 안에서 service → repository → entity 추가하면 끝나요. 다른 Gradle 모듈을 건드릴 일이 없어요.
