...
- **장점**: 진짜 프로세스 격리예요. 앱 A 의 OOM 이 앱 B 에 전혀 영향을 주지 않아요. 독립 배포 / 독립 스케일링 / 독립 JDK 업그레이드가 가능해요.
- **단점**: 10개 서비스 = 10개 배포 파이프라인 + 10개 Prometheus scrape target + 10개 Postgres connection pool + 10개 TLS 인증서 관리. **솔로 감당 불가 스케일**.
- **추가 문제**: "상태를 가진 공통 기능" (유저 테이블, 결제 테이블) 을 라이브러리로 해결하지 못해요. 라이브러리는 로직을 공유할 뿐 DB 를 공유하지 않으므로, 각 앱이 결국 자기 `users` 테이블을 복제해야 하고 드리프트가 발생해요.
- **탈락 이유**: 힘 A 를 정면 위반.
...
- **탈락 이유**: 힘 B 를 정면 위반. 솔로 개발자에게 특히 위험합니다 — PR 리뷰어가 없으니 실수가 누적돼도 아무도 지적하지 않습니다.
...
- **탈락 이유**: Option 1 의 운영 부담 + 라이브러리 유지 부담이 이중으로 붙어요.
...
- **힘 B 만족**: Gradle 모듈 선언이 의존을 물리적으로 차단해요. 사람 의지가 아닌 `GradleException` 이 얽힘을 막아 줘요.
...
canonical 규칙은 [`ArchitectureRules.java`](https://github.com/storkspear/template-spring/blob/main/common/common-testing/src/main/java/com/factory/common/testing/architecture/ArchitectureRules.java) 에 22개 public static final ArchRule 상수로 정의돼 있습니다. 규칙 1~5 가 이 결정과 직접 연관됩니다.
...
- **1 단계만 있으면**: `build.gradle` 선언은 깨끗한데 소스에서 `Class.forName()` 같은 reflection 으로 타인 모듈 클래스를 끌어오는 경우를 잡지 못해요.
- **2 단계만 있으면**: 소스 참조는 없지만 `runtime` / `compileOnly` 로 잘못 선언한 의존이 배포 아티팩트에 들어가는 경우를 빌드 시간에 잡지 못해요.
...
**모듈 경계가 기계에 의해 강제** — 5개 convention plugin + 22개 ArchUnit 규칙이 경계를 **사람 의지** 로부터 분리합니다. 솔로 개발자가 피곤한 날에도 실수로 얽힘이 들어가지 않습니다.
...
**`core-auth-impl` 의 `AuthController` 를 런타임에서 제거한 결정.**

**대안 분석** — `core-auth-impl` 안의 `AuthController` 를 `AuthAutoConfiguration` 의 `@Import` 로 **런타임 bean 등록** 하는 방식의 문제: 경로 `/api/core/auth/*` 를 모든 앱이 하나의 Controller 로 공유하게 됩니다.
...
이 복잡도가 "모듈러 모놀리스" 의 본래 목표 — **각 앱이 자기 경계 안에서 단순하게 동작** — 와 어긋납니다. 그래서 `AuthAutoConfiguration` 의 `@Import(AuthController.class)` 를 제거하고, `core-auth-impl/controller/AuthController.java` 는 런타임 bean 이 아니라 **`new-app.sh` 가 참조하는 스캐폴딩 소스** 로만 둡니다.
