...
- **단점**:
  ...
  - 진짜 필요해진 시점에 "이미 늦었나?" 라는 의심이 생길 수 있어요
- **채택 이유**:
...
**`/v1/` 접두사는 없음**. [`ApiEndpoints.java`](https://github.com/storkspear/template-spring/blob/main/common/common-web/src/main/java/com/factory/common/web/ApiEndpoints.java) 에서 grep 검증: `/v1/`, `/v2/` 문자열 0건.
...
본 ADR 은 "지금 안 한다" 만 하고 끝나면 불완전해요. "나중에 어떻게 할지" 를 함께 명시해야 YAGNI 가 안전한 선택이 돼요.
...
누적 비용이 적지 않아요. 버전 관리 **가치가 분명한 시점** 전에는 이 비용을 회피하는 게 합리적이에요.
