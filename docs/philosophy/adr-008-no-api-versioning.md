...
API 엔드포인트에 `/v1/` 을 붙이지 않습니다. 이유는 단순 — **버전 관리가 필요한 시나리오가 아직 아니기 때문**. 버전 관리의 본질은 "서버 측에서 통제할 수 없는 클라이언트" 를 위한 것이고, 우리는 서버와 Flutter 앱을 **같은 사람이 운영**합니다. API 가 바뀌면 서버 + 앱을 같이 배포하면 끝. 미래에 공개 API 나 멀티 버전 클라이언트가 생기면 그 시점에 Cloudflare 리버스 프록시 (`/api/v1/*` → `/api/*`) 또는 한 줄 `@RequestMapping` prefix 수정으로 도입. YAGNI 원칙의 전형.
...
- **탈락 이유**: 현재 이득이 0 이고 미래 이득도 한 줄 리팩토링으로 얻을 수 있어요. 업계 표준이라는 이유만으로는 채택 근거가 부족해요.
