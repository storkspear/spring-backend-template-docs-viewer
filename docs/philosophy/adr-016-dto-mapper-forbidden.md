...
- **탈락 이유**: Option 4 와 비교하면 명확한 이득이 없어요. 그냥 중간 단계일 뿐이에요.

### Option 4 — Entity 의 `to<Dto>()` 메서드 ★ (채택)

DTO 변환을 Entity 의 메서드로 제공해요. Mapper 클래스를 두지 않아요.
...
- **단점**:
  - **Entity 가 DTO 를 많이 알면 뚱뚱해져요** — User 에 `toSummary`, `toProfile`, `toAccount` 3 개 메서드가 있고, 5+ 개로 늘면 고민이 시작돼요.
  - **복잡한 매핑 (여러 Entity 조합) 을 Entity 에 담기 어려워요** — `AuthResponse(UserSummary + Tokens)` 같은 건 Entity 혼자서 만들 수 없어요.
- **완화**:
...
- 다른 개발자가 "아, 이 프로젝트는 Mapper 쓰는구나" 하고 `ExpenseMapper` 도 만들어요
...
DTO 변환은 **Entity 의 `to<Dto>()` 메서드** 로 제공해요. `*Mapper` 클래스는 ArchUnit r22 로 차단됩니다.
...
**ArchUnit 으로 표류 방지** — 규칙이 기계 강제되어 누군가 실수로 Mapper 를 만들 수 없어요. 장기 유지보수 안정성이 보장됩니다.
...
**복잡한 매핑 (여러 Entity 조합) 표현 제약** — `AuthResponse(UserSummary + AuthTokens + DeviceInfo)` 같은 조합은 Entity 혼자 만들 수 없어요. 해결: **Service 에서 조립** 합니다:
...
**생성자 패턴 혼용 가능성** — "`new UserSummary(...)` 로 직접 만드는 게 더 빠른 경우" 가 가끔 있어요 (테스트 fixture 등). 완화: 관행상 Service 코드에서는 `to<Dto>()` 를 우선하고, 테스트에서는 생성자 직접 호출을 허용해요. ArchUnit 이 이 구분까지 하지는 않아요.

### 감당 가능성 판단

단점들은 **현재 스케일 (엔티티 당 3 개 내외 DTO) 에서는** 실질적 문제가 아니에요. Entity 당 5+ 로 늘면 재검토 시그널이지만, 실제 대부분 도메인에서는 3 개 이하로 유지돼요. 장점 (호출 사이트 간결 + ArchUnit 강제 + 의존 없음) 이 압도적이에요.
...
**Entity `to<Dto>()` 패턴 구현**:
- [`User.java`](https://github.com/storkspear/template-spring/blob/main/core/core-user-impl/src/main/java/com/factory/core/user/impl/entity/User.java) — `toSummary()`, `toProfile()`, `toAccount()` 3개 메서드

**ArchUnit r22 강제**:
- [`ArchitectureRules.java`](https://github.com/storkspear/template-spring/blob/main/common/common-testing/src/main/java/com/factory/common/testing/architecture/ArchitectureRules.java) — r22 `NO_MAPPER_CLASSES`
- [`BootstrapArchitectureTest.java`](https://github.com/storkspear/template-spring/blob/main/bootstrap/src/test/java/com/factory/bootstrap/BootstrapArchitectureTest.java) — r22 실제 실행

**DTO 정의** (`core-user-api`):
- [`UserSummary.java`](https://github.com/storkspear/template-spring/blob/main/core/core-user-api/src/main/java/com/factory/core/user/api/dto/UserSummary.java)
- [`UserProfile.java`](https://github.com/storkspear/template-spring/blob/main/core/core-user-api/src/main/java/com/factory/core/user/api/dto/UserProfile.java)
- [`UserAccount.java`](https://github.com/storkspear/template-spring/blob/main/core/core-user-api/src/main/java/com/factory/core/user/api/dto/UserAccount.java)

**DTO 에서 검증/정규화 포함 static 팩토리 허용 예** (조건부 허용):
- [`core-auth-api/dto/`](https://github.com/storkspear/template-spring/tree/main/core/core-auth-api/src/main/java/com/factory/core/auth/api/dto) — `AuthTokens.of()` 같은 정규화 팩토리

**관련 문서**:
- [`convention/dto-factory.md`](https://github.com/storkspear/template-spring/blob/main/docs/convention/dto-factory.md) — 정적 팩토리 허용 규칙 상세
- [`structure/module-dependencies.md`](https://github.com/storkspear/template-spring/blob/main/docs/structure/module-dependencies.md) — `NO_MAPPER_CLASSES` ArchUnit 규칙
