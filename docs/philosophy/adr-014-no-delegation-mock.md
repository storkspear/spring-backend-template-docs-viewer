...
**구현 세부 버그를 곧바로 지목하지 못해요** — Port 계약 테스트가 실패하면 "어디서 틀렸는지" 를 추적하기 위한 디버깅이 필요해요. delegation mock 이라면 "verify 실패한 호출" 이 지목되지만 우리는 "최종 결과가 틀림" 만 알려줘요. 완화: 로그 + breakpoint 조합으로 추적합니다.
...
이건 **"JwtService 가 Jwts.builder() 를 호출했는가"** 를 검증하지 않아요. **"발급된 토큰을 검증하면 원본 값이 돌아오는가"** 라는 외부 관측 행위를 검증해요. 위임 체크가 아니에요.
...
**교훈**: 테스트 전략은 **코드 시작 전에** 선언해야 해요. "우선 쓰고 나중에 정리" 는 누적된 혼란으로 귀결돼요. 전략 문서 ([`testing-strategy.md`](../production/test/testing-strategy.md)) 가 코드만큼 중요해요.
...
**교훈**: 외부 시스템 격리가 필요해도 Mockito 가 최선은 아니에요. 도메인 의미의 fake 구현을 만들면 테스트 가독성 + 재사용성이 올라가요.
...
선제적으로 금지해두면 이 비용 자체가 발생하지 않아요. 금지의 가치는 **발생하지 않은 비용** 이라 측정이 어렵지만 실재해요.

**원칙**: 초기 비용이 낮은 패턴이 누적 비용은 클 수 있어요. 금지 결정은 **선제적** 으로 해야 효과가 있고, 누적된 후 제거하려면 몇 배 더 힘들어져요.
